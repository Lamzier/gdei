...
public class Update {
     ...
     * @param data 更改的数据
     * @param openid 修改的openid
     * @throws MysqlException 抛出数据库异常
     */
    public static void changeUser(Statement statement,Map<String,Object> data ,String openid) throws MysqlException{
        if (data.size() == 0){
            return;
        }
        ...
        int x = 0;
        for (Map.Entry<String, Object> linshi : data.entrySet()){
            key[x] = linshi.getKey();
            value[x] = linshi.getValue();
            //获取值
            stringBuilder.append(key[x]).append("=?");
            if (x < data.size() - 1){
                //如果不是最后一个
        ...
        String sql = "UPDATE mainUserinfo SET " + stringBuilder + " WHERE openid = '" + openid + "';";
        try {
    ...
    public static void initCreateTable(Statement statement){
        ...
        sql = "CREATE TABLE IF NOT EXISTS searchNotice(" +
                "id INT NOT NULL PRIMARY KEY AUTO_INCREMENT," +
                "openid VARCHAR(64) NOT NULL," +
                "nickName VARCHAR(64) NOT NULL," +
                "yuanxi VARCHAR(64) NOT NULL," +
                "time VARCHAR(64) NOT NULL," +
                "conTitle VARCHAR(64) NOT NULL," +
                "conCon VARCHAR(256) NOT NULL," +
                "conImage VARCHAR(256) NOT NULL," +
        ...
        sql = "CREATE TABLE IF NOT EXISTS recruitmentNotice(" +
                "id INT NOT NULL PRIMARY KEY AUTO_INCREMENT," +
                "openid VARCHAR(64) NOT NULL," +
                "nickName VARCHAR(64) NOT NULL," +
                "yuanxi VARCHAR(64) NOT NULL," +
